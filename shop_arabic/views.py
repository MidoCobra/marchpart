...
from main.models import HomePage_main_banners_WEBSITE, Offer_product, Offer_category
...
from shop.forms import ReviewProductForm, ContactUsForm, Merchant_Request_Form
...
def Home_arabic(request, category_slug=None):
    ...
    partBrands = Part_Brand.objects.all()
...
def Tires_arabic(request):
    tires = Product.objects.filter(tires=True, tire_rim_size__isnull=False, tire_height_num__isnull =False, tire_width_num__isnull=False)
    tireRim_sizes = Rim_size.objects.all().order_by('rim_size')
    tires_heights = Tire_height.objects.all().order_by('tire_height')
    tires_widths = Tire_width.objects.all().order_by('tire_width')

    rim_size_arabic = request.GET.get('rim_size_arabic', None)
    tire_height_arabic = request.GET.get('tire_height_arabic', None)
    tire_width_arabic = request.GET.get('tire_width_arabic', None)
    part_brand_arabic = request.GET.get('part_brand_arabic', None)
    latest_products_tires = Product.objects.filter(tires=True).order_by('-created_at')[:6]
    
    results = Product.objects.filter(
        hidden=False, tire_rim_size__rim_size= rim_size_arabic, tire_width_num__tire_width= tire_width_arabic, tire_height_num__tire_height= tire_height_arabic, 
        tires=True
        )
    ...
    context = {
        'tires': tires,
        'tireRim_sizes': tireRim_sizes,
        'tires_heights': tires_heights,
        'tires_widths': tires_widths,
   
        'results': results,
        'NoResults': NoResults,
        'products_paginator': products_paginator,
        'latest_products_tires': latest_products_tires,
    }

    return render(request, 'shop_arabic/product_arabic/tires_arabic.html', context)


def Liquids_brands_arabic(request):
    liquids = Product.objects.filter(liquids=True)
    brands = Part_Brand.objects.filter(liquid=True)
    latest_products_liquids = Product.objects.filter(liquids=True).order_by('-created_at')[:6]

    context = {
        'liquids': liquids,
        'brands': brands,
        'latest_products_liquids': latest_products_liquids,
    }

    return render(request, 'shop_arabic/product_arabic/liquid_brands_arabic.html', context)


def Liquids_types_arabic(request, id):
    brand = get_object_or_404(Part_Brand, id=id)
    liquid_types = Liquid_type.objects.filter(liquids_Types__part_brand__id=id).distinct()
    latest_products_liquids = Product.objects.filter(liquids=True).order_by('-created_at')[:6]
    ...
    return render(request, 'shop_arabic/product_arabic/liquid_types_arabic.html', context)


def Liquids_products_arabic(request, liquid_id, brand_id):
    brand = get_object_or_404(Part_Brand, id=brand_id)
    liquid_type = get_object_or_404(Liquid_type, id=liquid_id)
    liquid_products = Product.objects.filter(liquid_type__id=liquid_id, part_brand__id=brand_id)
    latest_products_liquids = Product.objects.filter(liquids=True).order_by('-created_at')[:6]
    ...
    return render(request, 'shop_arabic/product_arabic/liquid_products_arabic.html', context)

def PartBrand_arabic(request, id):
    brand = get_object_or_404(Part_Brand, id=id)
    brands = Part_Brand.objects.all().order_by('?')[:9]
    products = Product.objects.filter(part_brand__id = id)
    page = request.GET.get("page", 1)
    ...
    return render(request, 'shop_arabic/part-brand-products-arabic.html', context)

def ManfactureYears_Ajax(request):
    get_model = None
    manf_years = None
    response_data = {}
    if request.is_ajax and request.method == "POST":
        get_model = request.POST.get("modelSearch", None)
        manf_years = ManfactureDate.objects.filter(manfYears__model__name_arabic=get_model).distinct()
        all_years = []
        all_id = []
        for m in manf_years:
            all_id.append(m.id)
            all_years.append(m.manfacture_year)
        data = {
            "yearsQuerySet": sorted(all_years)
        }
        return JsonResponse(data)
...
def Merchant_Form_arabic(request):
    if request.method == 'POST':
        form = Merchant_Request_Form(request.POST or None, request.FILES or None)
        if form.is_valid():
            if request.recaptcha_is_valid:
                company_name = form.cleaned_data['company_name']
                purchasing_manager_name = form.cleaned_data['purchasing_manager_name']
                mobile = form.cleaned_data['mobile']
                email = form.cleaned_data['email']
                comercial_and_tax_pdf = form.cleaned_data['comercial_and_tax_pdf']
                quotation = form.cleaned_data['quotation']
                supplier = form.cleaned_data['supplier']
                buyer = form.cleaned_data['buyer']
                Merchant_Request.objects.create(
                    company_name=company_name, email=email, purchasing_manager_name=purchasing_manager_name,
                    mobile=mobile, comercial_and_tax_pdf=comercial_and_tax_pdf, quotation=quotation,
                ...
                current_site = get_current_site(request)
                subject = "Message From Merchant"
                messageBody = render_to_string(
                    "Merchant_Request_Message.html",
                    {
                        "domain": current_site.domain,
                        ...
                        'supplier' : supplier,
                        'buyer' : buyer,
                        # 'link' : "https://marchpart.com/admin/shop/merchant_request/" + 1 + "/change/",
                    },
                )
                to_emails = [
                    (email),
    ...
    return render(request, "merchant-form_arabic.html", context=context)

def AboutUsArabic(request):
    return render(request, "about-us-arabic.html")


def Offer_products_arabic(request, id):
    date = datetime.date.today()
    products = Offer_product.objects.filter(offer_category__id=id, valid_to__gte = date)
    random_products = Offer_product.objects.all().order_by('?')[:6]
    offer_category = get_object_or_404(Offer_category, id=id)
    all_offer_categories = Offer_category.objects.all()

    context = {
        'products' :products,
        'offer_category' :offer_category,
        'random_products' :random_products,
        'all_offer_categories' :all_offer_categories,
    }
    return render(request, 'offers_products_arabic.html', context)